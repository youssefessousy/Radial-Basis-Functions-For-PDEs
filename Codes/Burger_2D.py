"""
Created on Tue Aug 18 00:54:22 2020
...
""" 
    In this file, we call some functions from the file "Schemes2D.py", so make 
    sure the file "Schemes2D.py" exists in the same path where this file exists.
    
    The example we treat here using RBF method with Euler and Rung-Kutta's schemes
    for time stepping, is the 2D Burger equation given in the project of Radial 
    basis functions.
"""
...
a, b  =  0, 1  
e, f  =  0, 1                         # [a, b]*[e, f] est le domaine spatiale
t, T  =  0, 1                         # Le temps initial et le temps final
dt    =  0.005
Nx    =  20                           # Le nombre des points pris dans [a, b]
Ny    =  20                           # Le nombre des points pris dans [a, b]
N     =  Nx*Ny                        # Le nombre des points de collocation
c     =  10                          # The shape parameter pour MQ
sigma =  5                            # The shape parameter pour CS
cx    =  1                            # La vitesse suivant x
cy    =  1                            # La vitesse suivant y
D     =  0.005                         # Le coefficient de diffusion
...
def Burger2DSolExa(x, y, t, D) :
    return 1/(1 + np.exp((x + y - t)/(2*D)))
...
Uexa  =  Burger2DSolExa(X, Y, t, D)   # Solution exacte à l'instante t= 0
U     =  Uexa                         # Les deux solutions sont égaux à l'instante t = 0
u     =  U.reshape(N, 1)

B, Bx, Bxx = Schemes2D.choose_method(method)(r, s, w, shape)
B, By, Byy = Schemes2D.choose_method(method)(r, w, s, shape)
Bxy   =  Bxx + Byy
...
# fig1  =  plt.figure(1)
# ax1   =  Axes3D(fig1)
# surf1 =  ax1.plot_surface(X, Y, Uexa, cmap=cm.coolwarm, linewidth=0, antialiased=False)
# fig1.colorbar(surf1)
...
while t < T :
    ...
    Uexa  =  Burger2DSolExa(X, Y, t, D)
    
    uexa  =  Uexa.reshape(N, 1) 
            
    #########################  Solution Approchée  ###########################
                
    f = Schemes2D.Burger2D           # f sous forme de l'équation d'e Burger 2D
    
    u = Schemes2D.choose_schemes(schema)(dt, cx, cy, Ax, Ay, Axy, u, D, f)  # cx et cu ne jouent aucune role
                                                                            # lorsque f = Burger2D 
